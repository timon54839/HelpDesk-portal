...
export default async function RoomDetailPage({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      <Link href="/rooms" className="text-sm text-indigo-400 hover:text-indigo-300 hover:underline mb-4 inline-block">
        ← Zpět na místnosti
      </Link>
      <div className="card mb-6">
        <div className="flex items-center gap-4 mb-6">
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-emerald-900/80 text-3xl">
            🏫
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-100">{room.name}</h1>
            <p className="text-gray-400">Patro {room.floor}</p>
          </div>
        </div>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <dt className="text-sm font-medium text-gray-500">ID</dt>
            <dd className="mt-1 text-gray-400 font-mono text-xs break-all">{room.id}</dd>
          </div>
          <div>
          ...
          <div className="space-y-2">
            {roomDevices.map((device) => (
              <Link
                ...
                className="flex items-center gap-3 p-3 rounded-lg border border-gray-800 bg-gray-900 hover:bg-gray-800 hover:border-gray-700 transition-colors"
              >
